...
│                                                                   │
│  ┌──────────┐     ┌──────────────────────────────────────────┐  │
│  │  Nginx   │────→│  PM2 → Nuxt 3 (Nitro node-server) (:3400)│  │
│  │  :80     │     │         ├─ .output/server/index.mjs      │  │
...
| 层面 | 选型 | 说明 |
|------|------|------|
| 框架 | Nuxt 3 (Nitro) | node-server preset 输出，自带精简 node_modules 与静态资源 |
...
```bash
# Node.js 22（Nuxt 3 要求 Node 20.19+ / 22+）
...
首次部署时 `.output` 里没有 prisma CLI，需要手动同步数据库结构到生产数据库 `data/prod.db`：

```bash
cd /var/www/flow-calendar/app
npm install prisma@5.22.0 tsx --no-save
npx prisma db push
npx tsx prisma/seed.ts   # 可选：导入演示数据
```
...
`native` 对应 CI 环境（Debian/Ubuntu），`rhel-openssl-1.1.x` 对应 ECS（CentOS/RHEL）。两套引擎随 `.output` 一起打包到 ECS。
...
`nuxt.config.ts` 中配置了 `nitro.preset: "node-server"`，构建产物输出到 `.output/`（含 `server/index.mjs` 入口 + 精简 node_modules + `public/` 静态资源）。
...
# Nuxt 构建产物（保留 .output 目录名，与 ecosystem.config.js 的入口路径对齐）
cp -r .output deploy-package/.output

# Prisma schema（ECS 首次迁移数据库用）
cp -r prisma deploy-package/prisma

# package.json（让 ECS 上的 npx prisma/tsx 使用项目匹配版本）
...
# 删除开发数据库，防止覆盖生产数据
rm -f deploy-package/prisma/dev.db
```
...
deploy-package/
├── .output/
│   ├── server/index.mjs     # Nitro node-server 入口
│   ├── server/node_modules  # 精简依赖（含 Prisma Client）
...
```bash
# 写入 .env（Nuxt node-server 运行时不会自动加载 .env，需注入）
...
# PM2 热重载（首次自动启动）
# ecosystem.config.js 会自动从 .env 读取 DATABASE_URL 并注入进程环境
...
netstat -tlnp | grep 3400      # 看服务是否监听 3400
...
**根因**：Nginx 反代下，浏览器 `Origin` 是 `http://flow-calendar.gouxinjie.com`，但服务内部是 `http://127.0.0.1:3400`，`isTrustedMutationRequest()` 认为跨域。
...
### 7.5 生产环境报错 `Environment variable not found: DATABASE_URL`

**现象**：生产日志显示找不到 `DATABASE_URL`，接口返回 500。

**根因**：Nuxt 的 node-server 运行时**不会自动加载 `.env`**（`.env` 只在构建和开发时加载）。生产运行时需要环境变量在进程环境中。

**解决**：`ecosystem.config.js` 已内置 `loadEnvFile()`，启动时自动从 `.env` 读取 `DATABASE_URL` 并注入 PM2 进程。确保 `deploy.yml` 的 SCRIPT_AFTER 写入的 `.env` 包含正确的 `DATABASE_URL`（绝对路径）。

### 7.6 生产环境报错 `The table main.User does not exist`

**现象**：登录接口 500，日志显示表不存在。

**根因**：`DATABASE_URL` 用了相对路径（如 `file:./dev.db`），生产运行时解析到错误位置（新建了空库）。

**解决**：生产环境必须使用**绝对路径**的 `DATABASE_URL`，如 `file:/var/www/flow-calendar/data/prod.db`，并确保首次部署时已执行 `prisma db push` 建表。

### 7.7 PM2 errored 后未自动恢复
...
ls -la /var/www/flow-calendar/app/.output/server/index.mjs  # 确认部署产物时间戳
```
